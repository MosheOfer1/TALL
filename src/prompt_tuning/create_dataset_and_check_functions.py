...
def validate_translation(eng_input, eng_label, heb_label):
    ...
    return check_using_input_and_label_first_tran_label(eng_input, eng_label, heb_label)[0]
...
process_hebrew_translation_dataset("ynet_256k.txt", "ynet_256k.csv", log_version="1")

print("Processing complete. Results written to CSV.")


def create_hebrew(input_file, output_csv="ynet_600_hebrew.csv"):
    try:
        # Read the input file
        with open(input_file, 'r', encoding='utf-8') as file:
            lines = file.readlines()

        # Extract the first 150,000 sentences
        first_150k_sentences = lines[:150000]

        # Save the first 150,000 sentences to a temporary file
        temp_file = "first_150k_sentences.txt"
        with open(temp_file, 'w', encoding='utf-8') as output_file:
            output_file.writelines(first_150k_sentences)

        # Call the process_hebrew_translation_dataset function
        process_hebrew_translation_dataset(temp_file, output_csv, log_version="v1", buffer_size=50)
...
#input_csv = "ynet_output_matches.csv"
#copy_first_80000_lines(input_csv)

#create_hebrew("ynet_256k.txt")
#process_hebrew_translation_dataset("ynet_256k.txt", "ynet_256k.csv", log_version="v1", buffer_size=50)
